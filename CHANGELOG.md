...
- Repository is in **Step 3 — Core RAG orchestrator** (complete). Steps 0–3 are done. The next step is Step 4 — FastAPI serving layer (app + middleware, `/health`, `/chat` SSE endpoint, conversation manager, `/history`, `/feedback`, `/ingest`, response cache).
